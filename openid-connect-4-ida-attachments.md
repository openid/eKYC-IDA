...
`check_id`: Optional. Identifier referring to the verification or validation transaction that generated a particular attachment. When used in the context of an [@OpenID4IDA] response, the OP should ensure this matches a `check_id` contained within `check_method` when `check_method` needs to reference the embedded attachment.
