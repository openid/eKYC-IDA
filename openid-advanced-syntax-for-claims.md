...
relying parties and includes examples of how both features are used.

# Normative references

See section 7 for normative references.
...
### Syntax definition (*Review title*)
...
* `method`: Optional string. If provided, shall be one of `simple`, `schema`, or
  `exists`. If omitted, defaults to `exists`. Support for `schema` is optional
  for OPs and is discovered using metadata, as described in
  (#sao-metadata).
* `schema`: REQUIRED if `method` is `schema`, shall not be present otherwise. A
...
* `value` or `values`: Either `value` or `values` is REQUIRED if `method` is
  `simple`; shall not be present otherwise; `value` and `values` shall not be
  used together. For `value`, a valid claim value shall be provided which is
...
* `else`: Required. A string, either `abort` or `omit`, indicating the action
  ...
  shall be aborted. If `omit` is used, one or more elements shall be omitted from
  the response, as defined next.
* `what`: Optional if the value of `else` is `omit`, shall not be used
...
The `else` action shall be triggered when
...
Additionally, depending on the value of `method`, the following matches shall be
performed:
...
* `simple`: The value of the claim indicated by `loc` is matched against
  ...
  `nonexistent_family_name`. The `else` action shall be triggered if the value
  of the claim does not match the requested value or values.
* `schema`: The JSON schema `schema` element shall apply to the
  ...
  `verified_claims`. The `else` action shall be triggered if the schema does not
  apply.
* `exists`: No additional matches are performed.

The SAO rules provided shall be executed first for all `id_token` rules and then
for all `userinfo` rules.  Within each delivery type, the SAO rules shall be
executed in the order they are provided in the request. If an `abort` action is
triggered, the transaction shall be aborted and remaining SAO rules shall not be
executed. If an `omit` action is triggered, the respective elements shall be
omitted from the response; further SAO rules shall be executed. Note that `omit`
...
2. If the `birthdate` claim is not available or the birthdate is not a string
   ...
   from the response. Since the `verified_claims` element cannot be used
...
outside of `_asc`/`sao` shall be ignored when `_asc`/`sao` is present. This means
that RPs that define `_asc`/`sao` shall define all restrictions explicitly as SAO
rules.
...
* `selective_abort_omit_schema_supported`: Optional. Boolean value indicating
  ...
  the OP shall abort the transaction with an `invalid_request` error if a JSON
...
section 5.5.1 of [@!OpenID]. An OP supporting ASC but not SAO shall ignore the
`sao` key.
...
Each transformed claim is based off exactly one claim provided by the OP. For example, the claim `birthdate` can be used to derive a transformed claim for age verification (end-user is above a certain age) by applying a suitable chain of functions.  The number of functions in the chain can be limited by use of the optional OP metadata element `transformed_claims_max_depth`.

Each function takes one input value (the original claim's value or the output of the previous function) and produces one output value. Besides the input value, functions may only have static function arguments, typically zero or one.

## Example: age verification

The claim `birthdate`, a date, can be transformed into an integer using the function `years_ago`. This function outputs the number of years between the current date and the input date, rounded down. The resulting integer can be transformed using the function `gte` with the argument `18`. This function evaluates whether the input value is greater than or equal to the given argument. Its output is either `true` or `false`. The resulting transformed claim, representing whether the end-user is above 18 or not, may be aliased, for example `age_18_or_over`. This claim mayt be used within the OpenID Connect `claims` parameter instead of or together with the original claim, `birthdate`.
...
Example: In [@!IDA], the same `age_18_or_over` claim defined above is evaluated based on the 'verified claim' `birthdate` when used like this:
...
Any option available for normal claims may also be used with transformed claims. The evaluation of these options (e.g., a constraint defined using `value`) is always performed based on the transformed value.
...
This function may be used together with the `eq` operator or a restriction
...
may be at any location within `Input` unless further constrained by `RegEx`. Return `false` otherwise.
...
`transformed_claims_max_depth`: Optional. Integer value indicating the maximum number of functions in a chain of functions used to define a transformed claim. If this metadata value is omitted, the OP shall support chains of functions of any length.

`transformed_claims_max_count`: Optional. Integer value indicating the maximum number of transformed claims an RP can define, excluding any predefined transformed claims. If this is set to `0`, the RP can only use predefined transformed claims.  If this metadata value is omitted, the OP shall support any number of transformed claims.

## Error conditions
The following error conditions shall be checked by an OP, in this order:

1. If the definition of a transformed claim provided by an RP uses more than `transformed_claims_max_depth` function applications or the number of custom transformed claims exceeds `transformed_claims_max_count`, the OP shall abort the transaction with an `invalid_request` error. The `error_description` provided by the OP should indicate the location and nature of the error.
2. If an RP uses, in a definition for a transformed claim, a function not supported by the OP and therefore not listed in `transformed_claims_functions_supported`, or the wrong number of arguments, or a wrong type of argument, the OP shall abort the transaction with an `invalid_request` error. The `error_description` provided by the OP should indicate the location and nature of the error.
3. Each transformed claim is based on a single base claim, as expressed by the `claim` key in the definition of the transformed claim. In case this base claim is not known to the OP, or data is not available for this claim, or similar conditions, the transformed claim shall be treated the same as the base claim. For example, if the base claim is unknown to the OP, the transformed claim is handled as if it were an unknown claim as well. If an end-user choses not to release the base claim, or the base claim is not released to the RP for some other reason, the transformed claim shall not be released as well.

## UX and privacy considerations

The consent of the end-user is typically required before data is released by the OP to the RP. An OP is not expected to automatically parse and understand all potential combinations of transformation functions and their arguments in order to create a detailed consent prompt for the end-user.

OPs may use a number of strategies to ensure that end-user consent is always given in a meaningful way and to provide a good user experience (UX):

* Simple cases, for example the combination of `years_ago` and `gte(x)` applied
  to `birthdate` shown above, may be translated into a statement like "The RP
  wants to know that you are above age `x`". OPs may prepare a number of
  patterns to match against the RP's request for common use cases in their
  ecosystem.
* Since PTCs are predefined by the OP, the OP shall provide a
  meaningful consent text for all its PTCs.
* In all other cases, the OP may ask the end-user for
...
Moreover, some features in this specification are particularly suited for use cases of OpenID Connect where the RP pays for data received. In such use cases, integrity protection of the `claims` parameter is advised to avoid having the RP pay for data not requested.
...
* Authentication requests using transformed claims shall only be accepted by an OP if they are integrity-protected, unless `transformed_claims_max_count` is set to `0` in which case the OP may accept authentication requests without integrity protection and authentication. Since predefined transformed claims are defined by the OP, integrity protection and authentication is not required for their use.
...
Using a suitable security profile for OpenID Connect that includes authentication and integrity protection for the authentication request should be used, as this helps to ensure that the protection is not circumvented.

## Safe Execution of Transformation Functions
OPs shall ensure that all possible combinations of transformation functions and their respective arguments can be executed securely and without undesired side effects. In particular, for any function supported by the OP, the OP shall ensure that time and memory limits apply to avoid denial-of-service Attacks. For many functions, for example, comparison functions, this is usually inherent to the function itself. For other functions, execution time and complexity limits should be considered. For example, when applying regular expressions, regular expression DoS attacks (ReDoS) are a concern.
