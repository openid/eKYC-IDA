...
This specification defines the claims `age_is_at_least` and `age_is_at_most` to support age verification use cases in a privacy preserving manner.

`age_is_at_least`: is a JSON object consisting of the following fields:

* `age`: a JSON number as a non-negative integer that indicates a requested minimum age . 
* `on_date`: reference date in [@!ISO8601-2004] YYYY-MM-DD format used to determine the age of the End-User. 
* `result`:  a JSON boolean indicating whether the End-User has the minimum age as determined by the `age` field on the reference date.  

`age_is_at_most`: is a JSON object consisting of the following fields:

* `age`: a JSON number as a non-negative integer that indicates a requested maximum age . 
* `on_date`: reference date in [@!ISO8601-2004] YYYY-MM-DD format used to determine the age of the End-User. 
