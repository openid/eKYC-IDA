...
Note 1 to entry : A claims provider could be an OpenID provider, a verifiable claims issuer or other application component that provides verified claims.
...
The `trust_framework` value determines what further data is provided to the claims recipient in the `verification` element. A notified eID system under eIDAS, for example, would not need to provide any further data whereas a claims provider not governed by eIDAS would need to provide verification evidence in order to allow the claims recipient to fulfill its legal obligations. An example of the latter is a claims provider acting under the German anti-money laundering law (`de_aml`).
...
`type`: Required. Value shall be set to `document`.
...
`type`: Required. Value shall be set to `electronic_record`.
...
`type`: Required. Value shall be set to `vouch`.
